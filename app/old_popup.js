...
function addChannel(new_channels) {
    ...
    if(typeof new_channels == 'string' || new_channels instanceof String) {
        if(storage.channels.indexOf(new_channels) == -1) {
            storage.channels.push(new_channels);
            //streamer(new_channels);
        }
    } else if(Array.isArray(new_channels)) {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    } else for (var value in storage.channels) streamer(storage.channels[value]);
